...
export class Battle extends plugin {
  ...
  async Dajie(e) {
    ...
    const xingge=await readall('正在拍卖')
    if (xingge[0].竞拍次数!=4) {
    ...
    let now_level_idBB;
let now_mijinglevel_idBB;
let now_mijinglevel_idAA;
    ...
    now_mijinglevel_idBB = data.Levelmijing_list.find(item => item.level_id == playerBB.mijinglevel_id).mijinglevel_id;
     now_mijinglevel_idAA = data.Levelmijing_list.find(item => item.level_id == playerAA.mijinglevel_id).mijinglevel_id;
    //A是仙人，B不是仙人
   ...
   // 仙帝威严检查 - 被至高规则抹除
    if ( playerAA.mijinglevel_id< 21&&playerBB.mijinglevel_id >= 21) {
        e.reply([
            `【仙帝威严·不可侵犯】`,
            `你试图对${playerBB.名号}出手，`,
            `却不知对方乃路尽级仙帝！`,
        ...
        await redis.set('xiuxian:player:' + A + ':action', JSON.stringify(arr));
        return false;
    }
    //A是修仙者，B不是
