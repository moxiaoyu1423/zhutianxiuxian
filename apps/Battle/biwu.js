...
export class biwu extends plugin {
  ...
  async biwu(e) {
    ...
    let B = atItem[0].qq.replace('qg_',''); //后手
    B=await channel(B)
    if (A == B) {
      return false;
    }
  ...
  async battle(e, num) {
    ...
    while (A_player.当前血量 > 0 && B_player.当前血量 > 0) {
      msg_A = [`指令样式:#释放技能1第${cnt}回合,是否释放以下技能:`];
      for (const i in action_A.技能) {
        ...
        let cd =
          ...
          action_A.技能[i].cd;
        if (cd < 0) cd = 0;
        msg_A.push(`${i * 1 + 1}、${action_A.技能[i].name} cd:${cd}`);
      }
      await redis.set(
        'xiuxian:player:' + A_QQ[num].QQ + ':bisai',
        JSON.stringify(action_A)
      ...
      msg_B = [`指令样式:#释放技能1第${cnt}回合,是否释放以下技能:`];
      for (const i in action_B.技能) {
        ...
        let cd =
          ...
          action_B.技能[i].cd;
        if (cd < 0) cd = 0;
        msg_B.push(`${i * 1 + 1}、${action_B.技能[i].name} cd:${cd}`);
      }
      await redis.set(
        'xiuxian:player:' + B_QQ[num].QQ + ':bisai',
        JSON.stringify(action_B)
  ...
  async choice(e) {
    ...
    let msg = [];
    let us= e.user_id.toString().replace('qg_','')
    us= await channel(us);
    if (A_QQ.some(item => item.QQ == us)) {
      for (let j of A_QQ) {
        if (j.QQ == us) {
          code = code.slice(0, 3);
          for (let m in code) {
      ...
      e.reply(`本场战斗支持以下技能${msg}`);
      return false;
    } else if (B_QQ.some(item => item.QQ == us)) {
      for (let j of B_QQ) {
        if (j.QQ == us) {
          code = code.slice(0, 3);
          for (let m in code) {
  ...
  async release(e) {
    ...
    let us= e.user_id.toString().replace('qg_','')
    us= await channel(us)
    let action = await redis.get('xiuxian:player:' + us + ':bisai');
