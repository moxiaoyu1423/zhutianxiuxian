...
export class BOSS extends plugin {
  ...
  async CreateWorldBoss(e) {
    if (!e || e.isMaster) {
      // 记录群聊ID
      if (e?.group_id) {
        this.bossGroups.add(e.group_id);
      }
  ...
  async ShowDamageList(e) {
    ...
    if (await BossIsAlive()) {
      ...
      for (var i = 0; i < PlayerList.length; i++) {
        ...
        if (PlayerRecord.QQ[PlayerList[i]] == e.user_id.toString()) CurrentQQ = i + 1;
      }
  ...
  async WorldBossBattle(e) {
    ...
    if (data.existData('player', usr_qq)) {
      ...
      if (WorldBossStatus.Health <= 0) {
        ...
        e.reply(`[妖王] 结算:${usr_qq}增加奖励1000000`);
...
async function InitWorldBoss(e) {
    ...
    if (player_quantity == 0) {
        // 如果有e，则回复，否则记录日志
        if (e && e.reply) {
            e.reply('没有符合条件的玩家，妖王开启失败');
        } else {
            console.log('没有符合条件的玩家，妖王开启失败');
        }
        return -1;
    }
    ...
    // 如果有e，则回复，否则记录日志
    if (e && e.reply) {
        e.reply(`[妖王] 化神玩家总数：${player_quantity}`);
        e.reply(`[妖王] 生成基数:${X}`);
    } else {
        console.log(`[妖王] 化神玩家总数：${player_quantity}`);
        console.log(`[妖王] 生成基数:${X}`);
    }
    let Health = Math.trunc(X * 150 * player_quantity * 200); //血量要根据人数来
    let WorldBossStatus = {
        Health: Health,
        Healthmax: Health,
        KilledTime: -1,
...
async function GetAverageDamage(e) {
  ...
  for (var i = 0; i < File.length; i++) {
    ...
    if (level_id >= 42) {
            ...
            // 只在 e 存在且有 reply 方法时才回复
            if (e && e.reply) {
                e.reply(`[妖王] ${this_qq}玩家攻击:${temp[TotalPlayer]}`);
            }
            TotalPlayer++;
    }
  }
