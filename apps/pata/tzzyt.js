...
import { existplayer, ifbaoji, Harm,channel } from '../../model/xiuxian.js';
export class tzzyt extends plugin {
  ...
  async WorldBossBattle(e) {
    ...
    if (ZYTcs < 100) {
      Reward = 100 * ZYTcs + 100;
    } else if (ZYTcs >= 100 && ZYTcs < 200) {
      Reward = 200 * ZYTcs + 1000;
    } else if (ZYTcs >= 200) {
      Reward = 300 * ZYTcs + 1000;
    }
...
async all_WorldBossBattle(e) {
    ...
    // 唯一限制：100000层上限
    while (player.当前血量 > 0 && player.镇妖塔层数 < 50000) {
        let ZYTcs = player.镇妖塔层数;
        ...
        // 计算奖励
        let Reward = this.calculateReward(ZYTcs);
        if (Reward > 400000) Reward = 400000;
        // 简化版战斗模拟
        let battleResult = this.simulateSimpleBattle(player, bosszt);
        
        if (battleResult.win) {
            player.镇妖塔层数 += 5;
            cengshu += 5;
            lingshi += Reward;
            player.当前血量 = player.血量上限; // 恢复满血
        } else {
            player.当前血量 = 0;
            break;
        }
    ...
    e.reply([
        ...
        `通过层数：${cengshu}层`,
        `获得灵石：${lingshi}`,
        `当前镇妖塔层数：${player.镇妖塔层数}`,
        player.镇妖塔层数 >= 50000 ? '※ 已达到镇妖塔最高层数 ※' : ''
    ].join('\n'));
    return false;
}

// 极简战斗模拟
simulateSimpleBattle(player, bosszt) {
    let playerDamage = Harm(player.攻击, bosszt.Defence) + 
                      Math.trunc(player.攻击 * player.灵根.法球倍率);
    let bossDamage = Harm(bosszt.Attack, Math.trunc(player.防御 * 0.1));
    
    // 简单胜负判断
    if (playerDamage >= bosszt.Health) {
        return { win: true };
    } else if (bossDamage >= player.当前血量) {
        return { win: false };
    } else {
        // 如果双方都不能一击必杀，则玩家获胜（简化逻辑）
        return { win: true };
    }
}

// 奖励计算（保持不变）
calculateReward(ZYTcs) {
    if (ZYTcs < 100) return 100 * ZYTcs + 100;
    if (ZYTcs < 200) return 200 * ZYTcs + 1000;
    return 300 * ZYTcs + 1000;
}}
//发送转发消息
