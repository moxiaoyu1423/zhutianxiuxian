...
export class duanzao extends plugin {
  ...
  async all_clearthat(e) {
    ...
    for (let player_id of playerList) {
      let action = null;
      await redis.set(
        'zhutianxiuxian:' + player_id + ':action10',
    ...
    if (A == 1) {
      ...
      for (let item of newtripod) {
        if (user_qq == item.qq) {
          ...
          let action = null;
          await redis.set(
            'zhutianxiuxian:' + user_qq + ':action10',
            JSON.stringify(action)
          );
          e.reply('材料成功清除');
          return false;
  ...
  async givein(e) {
    ...
    const game_action = await redis.get(
      'zhutianxiuxian:' + user_qq + ':game_action'
    );
    //防止继续其他娱乐行为
    if (game_action == 0) {
      e.reply('修仙：游戏进行中...');
  ...
  async startit(e) {
    ...
    for (let item of newtripod) {
      if (user_qq == item.qq) {
        ...
        let action = await redis.get('zhutianxiuxian:' + user_qq + ':action10');
