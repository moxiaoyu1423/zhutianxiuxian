...
export class SecretPlaceTask extends plugin {
  constructor() {
    ...
    this.task = [
      ...
      {
        cron: this.set.travel_task, // 使用配置中的传送任务时间
        name: 'TravelTask',
        fnc: () => this.checkTravelTasks(),
      },
       {
            cron: '0 */30 * * * ?', // 每10分钟执行一次
            name: 'DaoshangCheckTask',
            fnc: () => this.checkDaoshangPunishment(),
        }
    ];
  }
  async Secretplacetask() {
    ...
    for (let player_id of playerList) {
      ...
      let fashanAction = await redis.get('xiuxian:player:' + player_id + ':action:fashan');
    ...
    // 检查是否为法身探索行动
    if (parsedAction.isFashan && parsedAction.Place_action === '0') {
        const end_time = parsedAction.end_time;
        ...
        if (now_time >= end_time) {
            ...
            // 获取地点信息
            const place = parsedAction.Place_address;
            ...
            // 法身必赢处理
            const resultMsg = await handleFashanSecretPlace(player, place);
            ...
            // 构建消息
            const finalMsg = [
                segment.at(Number(player_id)),
                resultMsg
            ];
            
            // 发送消息
            if (parsedAction.group_id) {
                await Bot.pickGroup(parsedAction.group_id).sendMsg(finalMsg);
            } else {
                await Bot.pickUser(player_id).sendMsg(finalMsg);
            }
            
            // 清除法身行动状态
            await redis.del('xiuxian:player:' + player_id + ':action:fashan');
...
let A_player = {
  ...
  当前血量: player.当前血量,
  暴击率: player.暴击率,
  灵根: player.灵根, // 复制整个灵根对象
  法球倍率: player.灵根.法球倍率, // 也可以保留这个，但最好统一用灵根对象里的
  仙宠: player.仙宠,
};
            let buff = 1;
                    ...
                    if (currentAptitudeIndex >= minAptitudeIndex&&!tianxuanbufa) {
                        ...
                        if (adjustedProbability < finalProbability) {
                            ...
                            if (isLearned) {
                                ...
                                await Add_player_学习功法(player_id, '天璇步法');
                                 await Write_player(usr_qq, player);
                            }
                        }
                    } else {
     ...
     if (putizi && putizi >= 1) { // 持有菩提子方可触发
         ...
         if (random < baseProbability + aptitudeBonus) {
             ...
             if (isLearned) {
                 ...
                 await Add_player_学习功法(player_id, '行字秘');
                  await Write_player(usr_qq, player);
             }
         } else {
             // 未触发时的道韵感悟
...
async checkTravelTasks() {
    // 获取所有传送任务
    const keys = await redis.keys('xiuxian:travel_task:*');
    ...
    for (const key of keys) {
      ...
      if (now >= task.end_time) {
        await this.completeTravelTask(task);
        await redis.del(key); // 删除任务
      }
    }
  }
  
 async completeTravelTask(task) {
    const { player_qq, to_location, to_name, travel_msg, player_name, group_id } = task;
    
    try {
        // 1. 更新玩家位置
        const player = await Read_player(player_qq);
        player.power_place = to_location;
        await Write_player(player_qq, player);
        
        // 2. 清除行动状态
        await redis.del(`xiuxian:player:${player_qq}:action`);
        await redis.del(`xiuxian:travel_task:${player_qq}`);
        
        // 3. 构建抵达消息
        const arriveMsg = [
            `【位面穿梭·完成】`,
            `${player_name}已成功抵达${to_name}!`,
            ...travel_msg,
            `位面穿梭完成，你现已身处${to_name}`
        ].join('\n');
        
        // 4. 发送消息
        if (group_id) {
            await Bot.pickGroup(group_id).sendMsg([
                segment.at(Number(player_qq)),
                arriveMsg
            ]);
        } else {
            await common.relpyPrivate(player_qq, arriveMsg);
        }
        
        return true;
    } catch (err) {
        console.error(`完成传送任务出错（玩家QQ：${player_qq}）：`, err);
        ...
        try {
            ...
            if (group_id) {
                ...
                await common.relpyPrivate(player_qq, errorMsg);
            }
        } catch (secondaryErr) {
            console.error('发送错误消息失败：', secondaryErr);
        }
        
        return false;
    }
}
async checkDaoshangPunishment() {
    try {
        console.log('【道伤检查】开始执行道伤惩罚检查...');
        ...
        for (const file of playerFiles) {
            try {
                totalPlayers++;
                const player_qq = file.replace('.json', '');
                const player = await Read_player(player_qq);
                const equipment = await Read_equipment(player_qq);
                if (!player) continue; // 跳过无效玩家数据
                if (!player.道伤 ) continue; // 跳过无道伤的玩家
                if (player.道伤 <= 0) continue; // 跳过无道伤的玩家
                
                const result = await this.applyDaoshangPunishment(player);
                if (result.affected) {
                    affectedCount++;
                    await Write_player(player_qq, player);
                    await Write_equipment(player_qq, equipment);
                    // 发送通知给玩家
                    await this.sendDaoshangNotification(player, result.message);
                }
            } catch (err) {
        ...
        console.log(`【道伤检查】完成！共检查${totalPlayers}名玩家，${affectedCount}名玩家受到道伤影响`);
    } catch (err) {
        console.error('道伤检查任务出错:', err);
    }
}
// 应用道伤惩罚
async applyDaoshangPunishment(player) {
    const result = {
        affected: false,
        message: ''
    };
    ...
    const messages = [`【道伤发作】${player.名号}的道伤开始恶化！`];
    const changes = [];
    ...
    // 根据道伤程度决定惩罚强度
    const daoshangLevel = Math.min(5, Math.floor(player.道伤)); // 最高5级
    
    if (daoshangLevel > 1) {
        messages.push('【道伤加剧·道基崩解】');
        messages.push('你原本的道伤在体内骤然恶化！');
        messages.push('紫府中大道根基寸寸崩裂，元神开始腐朽');
        messages.push('体内道则秩序链纷纷断裂：');
        messages.push('「噗——」一口道血喷出');
        messages.push('境界跌落！肉身衰败！秘境崩塌！仙古道痕消散！寿元枯竭！');
        messages.push('你以残存道力护住本源，');
        messages.push('勉强保住最后一丝根基（等级不会低于1级）');
        messages.push('但道伤已深入骨髓，仙路艰难...');
        player.寿元 = Math.max(1, Math.floor(player.寿元 * 0.3));
        player.元神 = Math.max(0, Math.floor(player.元神 - player.元神上限 * 0.2));
        player.元神上限 = Math.max(0, Math.floor(player.元神上限 - player.元神上限 * 0.2));
        changes.push(`寿元: ${player.寿元}`);
        changes.push(`元神: ${player.元神}/${player.元神上限}`);
        // 对每个属性随机扣除1-3级（不低于1级）
        levelProperties.forEach(prop => {
            if (player[prop] > 1) {
                // 道伤越重，惩罚越重
                const maxReduction = Math.min(daoshangLevel, 3);
                const reduction = Math.floor(Math.random() * maxReduction) + 1; // 随机1-maxReduction
                const newLevel = Math.max(1, player[prop] - reduction);
                const propName = getPropertyName(prop);
                changes.push(`${propName}: ${player[prop]} → ${newLevel} (跌落${reduction}级)`);
                player[prop] = newLevel;
            }
        });
    } else {
        messages.push('【道伤发作·道基不稳】');
        messages.push('你体内的道伤开始发作');
        messages.push('气血逆行，经脉紊乱，寿元锐减，神意崩开');
        messages.push('道基出现裂痕...');
         player.寿元 = Math.max(1, Math.floor(player.寿元 * 0.6));
         player.元神 = Math.max(0, Math.floor(player.元神 - player.元神上限 * 0.1));
        player.元神上限 = Math.max(0, Math.floor(player.元神上限 - player.元神上限 * 0.1));
        changes.push(`寿元: ${player.寿元}`);
        changes.push(`元神: ${player.元神}/${player.元神上限}`);
        // 基础惩罚（每个等级扣除1级，不低于1级）
        levelProperties.forEach(prop => {
            if (player[prop] > 1) {
                const originalLevel = player[prop];
                player[prop] = Math.max(1, originalLevel - 1);
                const propName = getPropertyName(prop);
                changes.push(`${propName}: ${originalLevel} → ${player[prop]} `);
            }
        });
    }
    ...
    // 道伤自然恢复（每10分钟减少0.1级，最低为0）
    const originalDaoshang = player.道伤;
    player.道伤 = Math.max(0, player.道伤 - 0.1);
    messages.push(`道伤减轻: ${originalDaoshang.toFixed(1)} → ${player.道伤.toFixed(1)}`);
...
// 发送道伤通知给玩家
async sendDaoshangNotification(player, message) {
    try {
        // 获取配置中的群组列表
            ...
            for (const groupId of notifyGroups) {
                try {
                    await Bot.pickGroup(groupId).sendMsg([
                        segment.at(Number(player.id)),
                        message
                    ]);
                    console.log(`[道伤通知] 群 ${groupId}: ${message}`);
                } catch (groupErr) {
                    console.error(`发送道伤通知到群 ${groupId} 失败[${player.id}]:`, groupErr);
                }
            }
        } else {
            // 如果没有配置群组，回退到私聊
            await common.relpyPrivate(player.id, message);
            console.log(`[道伤通知] 私聊 ${player.id}: ${message}`);
        }
    } catch (err) {
        console.error(`发送道伤通知失败[${player.id}]:`, err);
        
        // 最终回退到私聊发送
        try {
            await common.relpyPrivate(player.id, message);
            console.log(`[道伤通知] 私聊回退 ${player.id}: ${message}`);
        } catch (fallbackErr) {
            console.error(`私聊发送道伤通知失败[${player.id}]:`, fallbackErr);
        }
    }
}
  /**
...
async function handleFashanSecretPlace(player, weizhi) {
    // 法身必赢特效
    const victoryMsg = [
        `【法身·永恒不败】`,
        `${player.名号}的法身绽放无量仙光，`,
        `一缕神念自永恒未知之地降临${weizhi.name}！`,
        `诸天大道为之臣服，万古时空为之凝固！`,
        `秘境中所有宝物自动归附法身！`
    ];
    
    // 确保秘境等级存在
    const placeLevel = weizhi.level || 1;
    
    // 根据秘境等级确定奖励数量
    const rewardMultiplier = Math.max(1, Math.floor(placeLevel / 10));
    const itemCount = Math.max(1, 3 * rewardMultiplier); // 至少1件
    
    // 获取秘境奖励物品 - 兼容多种结构
    let allItems = [];
    
    // 情况1: 有直接items数组
    if (weizhi.items && Array.isArray(weizhi.items)) {
        allItems = weizhi.items;
    }
    // 情况2: 有one、two、three结构
    else if (weizhi.one || weizhi.two || weizhi.three) {
        if (weizhi.one && weizhi.one.length > 0) allItems = [...allItems, ...weizhi.one];
        if (weizhi.two && weizhi.two.length > 0) allItems = [...allItems, ...weizhi.two];
        if (weizhi.three && weizhi.three.length > 0) allItems = [...allItems, ...weizhi.three];
    }
    // 情况3: 物品列表直接是数组（如大千世界）
    else if (Array.isArray(weizhi)) {
        allItems = weizhi;
    }
    // 情况4: 没有找到物品列表
    else {
        console.error(`秘境 ${weizhi.name} 没有配置物品列表!`, weizhi);
    }
    ...
    if (allItems.length === 0) {
        ...
        console.warn(`使用默认物品列表`);
    }
    
    // 添加日志
    console.log(`秘境 ${weizhi.name} 物品列表:`, allItems);
    console.log(`准备添加 ${itemCount} 件物品`);
    ...
    for (let i = 0; i < itemCount; i++) {
        ...
        // 添加物品到纳戒
        await Add_najie_thing(player.id, item.name, item.class, 1);
    ...
    let rewardList = '';
    if (rewards.length > 0) {
        rewardList = rewards.map(item => `- ${item.name}`).join(" ");
    } else {
        rewardList = '（未获得任何物品）';
        console.error('未获得任何物品!');
    }
