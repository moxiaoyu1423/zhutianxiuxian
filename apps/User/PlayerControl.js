...
export class PlayerControl extends plugin {
  ...
  async chuGuan(e) {
    ...
    console.log(action)
    if(!("shutup" in action)){
      e.reply('出现错误!状态中不存在闭关状态,请留意最近进行的操作并向管理员反馈')
      return
    }
    ...
    if (e.isGroup) {
      await this.biguan_jiesuan(usr_qq, time, false, e.group_id); //提前闭关结束不会触发随机事件
    } else {
      await this.biguan_jiesuan(usr_qq, time, false); //提前闭关结束不会触发随机事件
    }
  ...
  async endWork(e) {
    ...
    usr_qq=await channel(usr_qq)
    let action = await this.getPlayerAction(usr_qq);
    if(!("working" in action)){
      e.reply('出现错误!状态中不存在降妖状态,请留意最近进行的操作并向管理员反馈')
      console.log(action)
      return
    }
    ...
    if (e.isGroup) {
      await this.dagong_jiesuan(usr_qq, time, false, e.group_id); //提前闭关结束不会触发随机事件
    } else {
