...
//保存文件夹
export async function dataall(neirong,diian) {
  let dir = path.join(`${__PATH.lib_path}/${diian}.json`);
  let new_ARR = JSON.stringify(neirong, '', '\t');
  fs.writeFileSync(dir, new_ARR, 'utf8', err => {});
  return;
}
//数量矫正, 违规数量改成1
...
export async function allwupin() {
  let B = [];
  let A = fs
    .readdirSync(__PATH.lib_path)
    .filter(file => file.endsWith('.json'));
  for (let item of A) B.push(item.substring(0, item.lastIndexOf('.')));
  return B;
}
